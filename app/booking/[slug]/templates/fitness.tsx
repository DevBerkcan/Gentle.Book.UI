...
import {
  withAlpha, getContrastColor, buildAnimVariants, INDUSTRY_EMOJI, ICON_MAP, ThemeTokenStyle, type TemplateProps,
} from "../_shared";
...
export function FitnessTemplate({
  ...
  const tokens = {
    ...
    "--tenant-cta-shadow": withAlpha(accent, 0.38),
  };
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#08090d] text-white">
      ...
      <div className="fixed inset-0 pointer-events-none" aria-hidden>
        ...
        <motion.div className="absolute -right-24 top-20 h-64 w-64 rounded-[40px] opacity-40 blur-3xl" style={{ background: "var(--tenant-accent)" }}
      ...
      <div className="relative mx-auto flex min-h-screen max-w-md flex-col px-5 py-8 pb-24">
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Zap size={18} style={{ color: "var(--tenant-accent)" }} />
          </div>
        ...
        <motion.div variants={container} initial="hidden" animate="visible" className="grid grid-cols-2 gap-3">
          {allLinks.map((link) => (
            <motion.a key={link.id} variants={item} href={link.url} target="_blank" rel="noopener noreferrer"
              ...
              <span className="mb-4 flex h-10 w-10 items-center justify-center rounded-2xl" style={{ background: withAlpha(accent, 0.18), color: "var(--tenant-accent)" }}>
