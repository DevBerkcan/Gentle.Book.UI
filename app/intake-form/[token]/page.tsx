...
export default function IntakeFormPage() {
  ...
  const canSubmit = preview?.fields.every((f) => !f.isRequired || (answers[f.id] ?? '').trim().length > 0) ?? false;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/public/intake-form/${token}`, {
        ...
        body: JSON.stringify({ answers: Object.entries(answers).map(([fieldId, value]) => ({ fieldId, value })) }),
      });
      if (res.ok) {
        setState('submitted');
      } else {
      ...
      <AnimatePresence mode="wait">
        ...
        {state === 'error' && (
          ...
          >
            ...
            <a href={supportConfig.mailto('Problem mit Anamnese-Link')}
        ...
        {(state === 'form' || state === 'submitting') && preview && (
          ...
          >
            <div className="px-8 pt-8 pb-6 text-center" style={{ background: `linear-gradient(135deg, ${PRIMARY}, #D8D7E2)` }}>
              ...
              <h1 className="text-xl font-bold text-white">Anamnesebogen</h1>
              <p className="text-white/65 text-sm mt-1">{preview.tenantName}</p>
            </div>

            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              {preview.fields.map((field) => {
                const options: string[] = field.fieldType === 'MultipleChoice' && field.optionsJson ? JSON.parse(field.optionsJson) : [];
                return (
                  <div key={field.id}>
